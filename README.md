...
- `ring_mode` 默认为 `custom`，支持两种模式：
  - `custom`：可通过 `fillet.radius_list` 定义倒角半径。  
    - 长度 = 1 或顶点数：所有环共用同一组半径；单值会自动扩展至顶点数长度。  
    - 长度 = `ring_num × 顶点数`：按圈分段精确控制每一层环的**内边界**倒角（例：3 层环、4 个顶点时，索引 `0~3`、`4~7`、`8~11` 分别作用于第 1~3 圈的内边界）。外边界半径会在生成阶段自动加上环宽 / 缩放偏移，保持同心。  
    - 长度 = `ring_num × 顶点数 × 2`：显式提供内外边界两份倒角列表。前一半（按圈分段）用于内边界，后一半用于外边界；此时外边界不再做自动修正，可完全独立控制。
...
1. 确保顶点坐标按逆时针顺序排列
2. 倒角半径可以超过相邻边长，系统会自动跨越并删除被覆盖的顶点；若日志出现 WARNING，说明大半径经过了折线，请确认几何是否合理
3. 缩放值应根据实际需求合理设置
4. 建议先使用小规模数据测试配置
...
### Via 倒角模式调整（2025-10-31）
- 解析阶段保留基础多边形的 `radius_list`，via 生成时对短列表进行继承比对。
- 与基础倒角一致时继续根据 `inner_zoom`/`outer_zoom` 自动修正半径，保持内外边界同心。
- 短列表若与基础图形不同，则视为自定义模式：内边界维持用户配置，外边界半径按缩放差自动推导，无二次修正。
- 显式双列表仍保持原有“内/外独立”语义。
- `tests/test_via_radius_series.py` 增补快照覆盖继承、显式、用户自定义三种场景。
