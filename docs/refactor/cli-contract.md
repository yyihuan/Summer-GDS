...
CLI 的输出 artifact 分两类：

- GDS：生产版图输出。
- 图片：GUI 预览和人工检查输出，例如 PNG，后续可扩展 SVG/PDF。

这两类输出是同级 output backend，不是“GDS 主流程 + debug 附件”的关系。
...
- GDS 和 PNG 都从同一批 final RegionObject 生成。
...
| `png` | `image_renderer.py` | GUI 预览和人工检查。 |
| `svg` | `image_renderer.py` | 后续可选，适合矢量预览。 |
...
为了 GUI 调用语义清晰，可以保留 `preview` 作为图片快捷命令：

```bash
summer-gds preview config.yaml --format png --out build/preview.png
```
...
| `4` | output backend 写出失败，例如 GDS 或 PNG 写出失败。 |
| `5` | CLI 参数错误。 |
...
sequenceDiagram
  ...
  GUI->>YAML: write config.yaml
  GUI->>CLI: summer-gds validate config.yaml
  CLI-->>GUI: validation result
  GUI->>CLI: summer-gds export config.yaml --format png --out build/preview.png --dry-run
  CLI-->>GUI: backend dry-run result
  GUI->>CLI: summer-gds export config.yaml --format png --out build/preview.png
  CLI->>PNG: write preview.png
  GUI->>CLI: summer-gds export config.yaml --format gds --out build/layout.gds
  CLI->>GDS: write layout.gds
  CLI-->>GUI: summary / errors
```
